## This script generates simulation scenarios for normal data
## This is very similar to the ordinal data scenarios, but has different treatment effects
## to keep Cohen's D around 0.5, and the baseline response effects were adjusted a
## bit in the ordinal scenarios to be able to keep the response values within the
## support.
...
target_total_effect <- 4.25 # target treatment effect on total score
...
# ## SDs of treatment effects in each scenario
# lapply(treatment_effects[[2]], sd)
# 
# ## SDs of baseline effects in each scenario
# lapply(baseline_effects, sd)

write_rds(simulation_scenarios, file=here("Output", "Simulation Scenarios", "simulation_scenarios_mvn.rds"))
